...
public class BeaconConnection
{
	...
	private final JaaleeService estimoteService;
	...
	public BeaconConnection(Context context, Beacon beacon, ConnectionCallback connectionCallback)
	{
		...
		this.estimoteService = new JaaleeService();
	...
	{
		if ((!isConnected()) || (!this.estimoteService.hasCharacteristic(JaaleeUuid.UUID_NORMAL_CHAR))) {
		...
		BluetoothGattCharacteristic uuidChar = this.estimoteService.beforeCharacteristicWrite(JaaleeUuid.UUID_NORMAL_CHAR, writeCallback);
	...
	{
		if ((!isConnected()) || (!this.estimoteService.hasCharacteristic(JaaleeUuid.ADVERTISING_INTERVAL_CHAR))) {
		...
		BluetoothGattCharacteristic intervalChar = this.estimoteService.beforeCharacteristicWrite(JaaleeUuid.ADVERTISING_INTERVAL_CHAR, writeCallback);
	...
	{
		if ((!isConnected()) || (!this.estimoteService.hasCharacteristic(JaaleeUuid.POWER_CHAR))) {
		...
		BluetoothGattCharacteristic powerChar = this.estimoteService.beforeCharacteristicWrite(JaaleeUuid.POWER_CHAR, writeCallback);
	...
	public void writeMajor(int major, WriteCallback writeCallback)
	{
		...
		BluetoothGattCharacteristic majorChar = this.estimoteService.beforeCharacteristicWrite(JaaleeUuid.MAJOR_CHAR, writeCallback);
	...
	public void writeMinor(int minor, WriteCallback writeCallback)
	{
		...
		BluetoothGattCharacteristic minorChar = this.estimoteService.beforeCharacteristicWrite(JaaleeUuid.MINOR_CHAR, writeCallback);
	...
	private BluetoothGattCallback createBluetoothGattCallback() {
		return new BluetoothGattCallback()
		{
			...
			public void onCharacteristicWrite(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status)
			{
				...
				} else if (JaaleeUuid.ESTIMOTE_SERVICE.equals(characteristic.getService().getUuid()))
	...
	public static class BeaconCharacteristics
	{
    	...
    	public BeaconCharacteristics(JaaleeService estimoteService, VersionService versionService)
    	{
